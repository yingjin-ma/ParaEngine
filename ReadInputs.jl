...
function readxyz2gauss(suit)
    ...
    for ifile in filelist
        ...
        elseif uppercase(split(ifile,".")[2]) == "PDB"  
            push!(names,split(ifile,".")[1])
            ifile = string(suit,"/",ifile)
            ifrag = ifrag + 1
            println("ifile : ", ifile, " || PDB  ==>  GJF " )
            i = 0
            open(ifile,"r") do xyzread
                readline(xyzread)
                #cline = readline(xyzread)
                ...
                while !eof(xyzread)
                    ...
                    ntmp   = length(sline)
                    # println("ntmp : ", ntmp)
                    if ntmp > 9 && sline[1]!="MASTER"
                        i = i + 1
                        dx = parse(Float64,sline[6])
                        dy = parse(Float64,sline[7])
                        dz = parse(Float64,sline[8])
                        push!(atomlist,ATOMS(i+natoms,ifrag,icharge,sline[3],(dx,dy,dz),0.0))
                    end
                end
            end
            natoms = natoms + i            
...
function readgauss(suit,par)
    ...
    for ifile in filelist
        ...
        txt = replace(txt,"%mem=2GB" => "%mem=48GB", "%nproc=20" => "%nproc=24", "%nproc=10" => "%nproc=24", "MP2/" => "MP2=FullDirect " )

        if uppercase(par)=="BABEL"
            txt = replace(txt,"!Put Keywords Here, check Charge and Multiplicity." => "%mem=48GB\n%nproc=24", "%mem=108GB" => "%mem=48GB")
            println("ifile (from BABEL) : ", ifile, " || change proc to 24 cores in ERA" )  
        else 
            println("ifile : ", ifile, " || change proc to 24 cores in ERA"  )
        end

        # rewrite the infile with keyword change (above)
        ...
        open(ifile,"r") do molread
            # skip the 6 lines that generated by gridmol/babel
            readline(molread)
            readline(molread)
            readline(molread)
            readline(molread)
...
#                   println(uppercase(sline[1])) 
                    ...
                    if uppercase(sline[2]) == "GAUSSIAN"
                        ...
                        elseif uppercase(sline[3]) == "BABEL"
                            global runtype2 = "BABEL"
                        elseif uppercase(sline[3]) == "GJF" || uppercase(sline[3]) == "COM"
                            global runtype2 = "G09" 
                        end  
                    end  
...
#                    println(" uppercase[2] [3] : ", uppercase(sline[2]), uppercase(sline[3]))
                ...
                if uppercase(sline[1]) == "SUIT"
                    ...
                    if runtype2 == "G09" || runtype2 == "G16"
                        ...
                        readgauss(targetsuit,"TEMP")
                    elseif runtype2 == "XYZGAU"
                        global targetsuit = sline[2] 
                        readxyz2gauss(targetsuit)                      
                    elseif runtype2 == "BABEL"
                        global targetsuit = sline[2] 
                        readgauss(targetsuit,"BABEL")                         
                    else
                        global targetsuit = sline[2] 
