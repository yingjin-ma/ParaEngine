...
# e.g.
#       julia  Timing_analysis_sequeue.jl  ../ParaEngine-git-Gaussian/P38-M062x-ALL  ../ParaEngine-git-Gaussian/LB-LSTM_P38_50_M062x   MLSLB-ONLY
...
if !isdir(lbfile)
    println("The lbfile $(lbfile) is not exist") 
    exit("Stopped. Reason: $(lbfile) is not exist.")
else
    println("Monitor file : ", lbfile )
end
...
if length(lbtask) == length(nodetask4) == length(nodetask3) == length(nodetask2) == length(nodetask1)
     println(" ==>  DIM match ")
     println(" ==>  DIM match ")
     println(" ==>  DIM match ")
end 
...
for i in 1:length(fraglist)
    ...
    for j in 1:length(lbtask) 
        idx1pos = findfirst(==(fraglist[i].idx),lbtask[j])
        if idx1pos != nothing
           idx0 = j
           idx1 = idx1pos
           #println("break ",j, "  ", idx1pos)
    ...
    #println("LB : idx0, idx1 : ", idx0, " ", idx1)
...
MNelems = Array{FRAGS,2}(undef,length(nodetask1),10*length(lbtask[1])) 

println(" ============== auto check if LB file shoud be used ==============  ")
icx = []
for i in 1:length(fraglist)
...
println(length(nodetask1), " - ",length(lbtask[1]))
for i in 1:length(fraglist)
